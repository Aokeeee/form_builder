<template>
  <n-modal
    ...
    :mask-closable="false"
  >
    <div class="json-wrapper" v-if="Object.keys(jsonData).length">
      <n-scrollbar style="max-height: 500px">
        <JsonViewer boxed copyable :value="jsonData" :expand-depth="5">
          <template #copy="{ copied }">
            <n-button text type="primary"> {{ copied ? 'copied!!' : 'copy' }}</n-button>
          </template>
        </JsonViewer>
      </n-scrollbar>
    </div>
    <template #action>
      <n-space>
        ...
        <n-button type="primary" @click="handleExportForJson">下载文件</n-button>
      </n-space>
    </template>
  </n-modal>
</template>
